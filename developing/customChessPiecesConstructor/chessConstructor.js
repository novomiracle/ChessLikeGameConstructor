...
movementCanvas.addEventListener("click", (e) => {
	...
	if(!thePath.moves.some((move)=>{
		return move.x ==  Math.floor(e.offsetX / (proportion * pixelSize)) - Math.floor(movementCanvas.width / (2 * pixelSize)) &&
			move.y == Math.floor(e.offsetY / (proportion * pixelSize)) - Math.floor(movementCanvas.height / (2 * pixelSize))
	})){
...
movementCanvas.addEventListener("contextmenu", (e) => {
	...
	if (!(ctx.getImageData(Math.floor(e.offsetX / (proportion*pixelSize)) * pixelSize, Math.floor(e.offsetY / (proportion*pixelSize)) * pixelSize, 1, 1).data[0] == 0 && ctx.getImageData(Math.floor(e.offsetX / (proportion*pixelSize)) * pixelSize, Math.floor(e.offsetY / pixelSize) * pixelSize, 1, 1).data[1] == 0 && ctx.getImageData(Math.floor(e.offsetX / (proportion*pixelSize)) * pixelSize, Math.floor(e.offsetY / pixelSize) * pixelSize, 1, 1).data[2] == 0 && ctx.getImageData(Math.floor(e.offsetX / (proportion*pixelSize)) * pixelSize, Math.floor(e.offsetY / pixelSize) * pixelSize, 1, 1).data[3] == 255)) {
		...
		if (thePath.type == "slide") {
			...
			thePath = thePath.moves.filter(x => !removeThose.includes(x));
		} else {
			...
			thePath = thePath.splice(theSquareInArrayIndex, 1)
		}
...
for (let i = 0; i < draggableBlockTemplates.length; i++) {
	...
	draggableBlockTemplates[i].addEventListener("dragstart", function (e) {
		e.target.id = idForBB;
		idForBB++
		...
		buildingContainers[i].addEventListener("drop", function (ev) {
			...
			if (ev.target.parentElement.classList[0] != "block_template" && (ev.target.childElementCount == 0 || ev.target.classList.contains("AF_BB")) || ev.target.classList.contains("BB_storage")) {
				...
				if (document.getElementById(data).classList[0] == "block_template") {
					...
					clonneBB.addEventListener("dragstart", function (e) {
						e.dataTransfer.setData("text", e.target.id);
						const rect = e.target.getBoundingClientRect();
						offsetX = e.x - rect.x;
						offsetY = e.y - rect.y;
					...
					if (ev.target.classList.contains("BB_storage")) {
						console.log(offsetX,ev.x,parseInt(getComputedStyle(ev.target).left))
						clonneBB.style.left = ev.x- offsetX - parseInt(getComputedStyle(ev.target).left)+ "px"
						clonneBB.style.top = ev.y- offsetY - parseInt(getComputedStyle(ev.target).top) + "px"
					}
					//adding the clone
					...
					if (ev.target.classList.contains("building_containers") && clonePrevention) {
						...
						clonneBB.addEventListener('contextmenu', function (e) {
							e.preventDefault();
							context = clonneBB
							contextMenu.style.top = e.clientY - parseInt(getComputedStyle(clonneBB.parentElement).top)+ "px"
							contextMenu.style.left = e.clientX - parseInt(getComputedStyle(clonneBB.parentElement).left)+ "px"
					...
					if (ev.target.classList.contains("building_containers")) {
						...
						if (ev.target.classList.contains("BB_storage")) {
							document.getElementById(data).style.left = ev.clientX-offsetX - parseInt(getComputedStyle(ev.target).left) + "px"
							document.getElementById(data).style.top = ev.clientY-offsetY - parseInt(getComputedStyle(ev.target).top) + "px"
						}
					}
				}
			}
	...
	for (let i = 0; i < el.children.length; i++) {
		...
		switch (el.children[i].classList[1]) {
			...
			case "if_else":
				...
				break;
			case "capture":
				console.log(elementsIntoCodeCondition(element.findClass("building_containers")))
			...
			case "create":
				...
				break;
			case "set_moves":
				let nums = "bc:<" + elementsIntoCodeCondition(element.findClass("building_containers"))+">"
				answer += `set_moves(${nums});`
			default:
				break
}
	}
...
dublicateButton.addEventListener("click", (e) => {
	if (context.parentElement.classList.contains("BB_storage")) {
		...
		clone.addEventListener('contextmenu', function (e) {
			e.preventDefault();
			context = clone
			contextMenu.style.top = e.clientY - parseInt(getComputedStyle(clone.parentElement).top)+ "px"
			contextMenu.style.left = e.clientX - parseInt(getComputedStyle(clone.parentElement).left)+ "px"
			contextMenu.classList.add("visible")
		}, false)
