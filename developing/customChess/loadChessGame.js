		...
		fileReader.onload = (e) => {
			...
			theGame.piecePrototypes.forEach((pr) => {
				for (let i = 0; i < pr.movement.length; i++) {
					...
					console.log("af1",additionalEffectToFunction(pr.movement[i].applyAdditionalEffect))
...
function additionalEffectToFunction(str,func = ""){
	...
	switch(name){
		...
		case "if_else":
			...
			break
		case "capture":
			func += `clearSquare(${val(args[0])});`
			break
		case "move_to":
			func += `movePiece(chessBoard.children[findSquare(${args[0]})],findSquare(${args[1]}));`
			break
		case "create":
			func +=`createChessPiece('${args[0]}',${args[1]},sameNotColor(${args[2]}));`
			break
		default:
			throw new Error('unknown command');
	...
	if(str == ""){
		...
		additionalEffectToFunction(str,func)
	}
}
...
function math_func(el1, sign, el2) {
	if (el1.isObject && el2.isObject()) {
		return `{x:${el1}.x ${sign} ${el2}.x,y:${el1}.y ${sign} ${el2}.y}`
	} else if (!el1.isObject && el2.isObject()) {
		return `{x:${el1} ${sign} ${el2}.x,y:${el1} ${sign} ${el2}.y}`
	} else if (el1.isObject && !el2.isObject()) {
		return `{x:${el2} ${sign} ${el1}.x,y:${el2} ${sign} ${el1}.y}`
	} else {
		return `${el2} ${sign} ${el1}`
	}
}
